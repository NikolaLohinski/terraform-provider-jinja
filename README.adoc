...
:tfplugindocs-url: https://github.com/hashicorp/terraform-plugin-docs
...
[NOTE]
====
The `{docs-folder}/` folder is generated using link:{tfplugindocs-url}[`tfplugindocs`] with the `{templates-folder}/` folder. To regenerate it:

* Install `tfplugindocs` by following the link:{tfplugindocs-url}#installation[official documentation]
* Run it as follows:
+
[source,shell]
----
